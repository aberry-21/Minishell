/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   prompt_utils.c                                     :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: aberry <aberry@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/08 18:59:51 by aberry            #+#    #+#             */
/*   Updated: 2021/02/08 19:03:43 by aberry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static t_line	*ft_move_string(t_line *pattern)
{
	char		*offset;
	size_t		index;
	t_line		*content_line;

	content_line = 0;
	if (!(content_line = ft_line_create_line(pattern)))
		return (content_line);
	offset = ft_strrchr(content_line->string, '/');
	if (offset)
	{
		index = offset - content_line->string;
		while (index > 0)
		{
			ft_line_del_chr(content_line, 0);
			index--;
		}
		ft_line_del_chr(content_line, 0);
	}
	return (content_line);
}

int				ft_prompt_command_add_to_lst(t_list *tmp_lst, t_list **res)
{
	t_line		*line_add_lst;

	while (tmp_lst)
	{
		if (!(line_add_lst = ft_move_string((t_line*)(tmp_lst->content))))
			return (1);
		if (!ft_lstadd_back_content(res, line_add_lst))
		{
			ft_line_del(line_add_lst);
			return (1);
		}
		tmp_lst = tmp_lst->next;
	}
	return (0);
}

t_list			*ft_find_command_in_path(char *path, t_line *line)
{
	t_list		*tmp_lst;
	t_line		*tmp_line;

	tmp_lst = 0;
	if (!(tmp_line = ft_line_create_str(path)))
		return (tmp_lst);
	if (!ft_line_add_chr(tmp_line, '/'))
	{
		ft_line_del(tmp_line);
		return (tmp_lst);
	}
	if (!(ft_line_add_line(tmp_line, line)))
	{
		ft_line_del(tmp_line);
		return (tmp_lst);
	}
	if (!(ft_line_add_chr(tmp_line, '*')))
	{
		ft_line_del(tmp_line);
		return (tmp_lst);
	}
	tmp_lst = ft_parse_wildcard(tmp_line);
	ft_line_del(tmp_line);
	return (tmp_lst);
}
