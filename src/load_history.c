...
/*   Updated: 2021/05/28 04:10:53 by telron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_feal_command(t_shell *config, int fd)
{
	...
	while (0 < read(fd, &buffer, 1))
	{
		if (buffer == '\n')
			...
			ft_input_keys_any_key(config, buffer);
	}
}
...
void		ft_load_history(t_shell *config)
{
	...
	while ((ent = readdir(dir)) != NULL)
	{
		if (!ft_strcmp(ent->d_name, ".") || !ft_strcmp(ent->d_name, ".."))
			continue ;
		ft_dlstadd_sort(&files, ft_dlstnew(ft_strdup(ent->d_name)), (int (*)(void *, void *))ft_filename_cmp);
	}
	closedir(dir);
