...
static t_line		*ft_get_argum(\
							t_shell *config,\
							t_dlist *argum)
{
	t_line			*line;
	t_parse_element	*element;
	t_line			*has_not_error;

	line = ft_line_create_str("");
	has_not_error = line;
	while (argum && has_not_error)
	{
		element = argum->content;
		if (element->type == IMMUTABLE_CODE)
			has_not_error = ft_line_add_line(\
								line, element->content.immutable.line);
		else if (element->type == STRING_CODE)
			has_not_error = ft_line_add_line(\
								line, element->content.string.line);
		else if (element->type == VARIABLE_CODE)
			has_not_error = ft_variable(\
								config, line, element->content.variable.line);
		argum = argum->right;
	}
	if (!has_not_error)
		ft_line_del(line);
	return (has_not_error ? line : 0);
}
