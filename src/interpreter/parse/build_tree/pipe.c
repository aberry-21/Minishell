...
/*   Updated: 2021/02/20 03:56:10 by telron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			char is_end)
{
	...
	if (!(element = ft_parse_build_redirection(config, *buffer)))
			...
			t_dlist *tokens)
{
	...
	while (tokens)
	{
		...
		{
			if (!ft_command_add_buffer(config, &command, &buffer, 0))
				return (0);
		}
		else
		{
	...
	return (ft_command_add_buffer(config, &command, &buffer, 1) ?
				ft_create_pipe_tree(command) : 0);
}
