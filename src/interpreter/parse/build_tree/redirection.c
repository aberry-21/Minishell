...
/*   Updated: 2021/02/20 03:58:55 by telron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			int type)
{
	...
	result = ft_parse_redirection_create(ft_parse_build_redirection(config, *command_tokens), file_name, type);
	ft_dlstclear(command_tokens, (void (*)(void*))ft_parse_element_destructor);
	return (result);
}

t_parse_element			\
	*ft_parse_build_redirection(\
			t_shell *config,\
			t_dlist *tokens)
{
	t_dlist			*dlist;
