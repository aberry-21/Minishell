...
/*   Updated: 2021/02/06 07:59:37 by telron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_input(t_shell *config)
{
	...
	config->input.mode = MODE_DEFAULT;
	...
	while (1)
	{
		...
		if (0 >= ret_read)
		{
			if (ft_memcmp(&(config->saved_winsize), &g_winsize, sizeof(struct winsize)))
			{
				if (ft_memcmp(&(config->saved_winsize.ws_col), &g_winsize.ws_col, sizeof(g_winsize.ws_col)))
					write(1, "\033[2J", 4);
